...
export class LambdaRunnerProvider extends BaseProvider implements IRunnerProvider {
  ...
  /**
   * Shared state machine fragment that runs any Lambda provider, reading per-provider configuration generated by
   * {@link _runnerConfig} from `$.providerParams`. Mirrors what LambdaInvoke used to render when each provider had
   * a dedicated state.
   *
   * @internal
   */
  ...
  /**
   * Runtime configuration for the shared Lambda family fragment.
   *
   * @internal
   */
  _runnerConfig(): any {
