...
export class EcsRunnerProvider extends BaseProvider implements IRunnerProvider {
  /**
   * Shared state machine fragment that runs any ECS provider, reading per-provider configuration generated by
   * {@link _runnerConfig} from `$.providerParams`. Mirrors what EcsRunTask used to render when each provider had a
   * dedicated state. ECS task definitions use bridge networking, so there is no NetworkConfiguration.
   *
   * @internal
   */
  public static _stateMachineFragments(scope: Construct): FamilyFragmentBranch[] {
    return [{
      condition: stepfunctions.Condition.and(
        ...
        stepfunctions.Condition.stringEquals('$.providerParams.family', 'ecs'),
      ),
      chainable: new stepfunctions.CustomState(scope, 'ECS Runner', {
        stateJson: {
          Type: 'Task',
   ...
   * @internal
   */
  readonly _runnerFamilies: string[] = ['ecs'];
  ...
  /**
   * Runtime configuration for the shared ECS family fragment.
   *
   * @internal
   */
  _runnerConfig(): any {
    // placement strategies/constraints are static per provider, so pass them ready-made in the API shape
    // expected by ecs:runTask (first letter of each key uppercased), exactly like EcsEc2LaunchTarget renders them
