...
function ec2RunInstancesState(spot: boolean): any {
  ...
  return {
    ...
    Parameters: {
      ...
      // both userdata templates are guaranteed in $.consts by _stepFunctionConstants(); see
...
export class Ec2RunnerProvider extends BaseProvider implements IRunnerProvider {
  ...
  public _stepFunctionConstants(): Record<string, string> {
    // always include both templates; the shared EC2 fragment selects one by index at runtime and
    // references both in States.Array(), so both must exist whenever any EC2 provider exists
    return {
