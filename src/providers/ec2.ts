 ...
 */
export class Ec2RunnerProvider extends BaseProvider implements IRunnerProvider, IParameterizedProvider {
  public static readonly _FAMILY = 'ec2';

  /**
   * Static strings the shared EC2 fragments need at `$.config`. Both userdata templates are always included
  ...
  /**
   * Shared state machine fragment that runs any EC2 provider, reading per-provider configuration generated by
   * {@link _runnerConfig} from `$.providerParams`. Spot and on-demand share one state; spot options come from the
   * config.
   *
   ...
   * exact States.Format() semantics (including `\{` escaping) the templates were written for.
   *
   * @internal
   */
      ...
      stateJson: {
        ...
        Parameters: {
          ...
          'UserData.$': 'States.Base64Encode(States.Format(' +
            'States.ArrayGetItem(States.Array($.config.ec2UserDataLinux, $.config.ec2UserDataWindows), $.providerParams.userDataTemplateIdx), ' +
  ...
  _runnerConfig(): any {
    const base = {
      family: 'ec2',
      // the provider that actually launches the instance, which is not $.provider when this config was reached
      // through a composite; used for the GitHubRunners:Provider tag
      ...
      // empty (not missing) for on-demand: the shared state reads this path unconditionally, and a missing
      ...
      // index into States.Array($.config.ec2UserDataLinux, $.config.ec2UserDataWindows)
