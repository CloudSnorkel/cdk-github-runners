...
/**
 * Grant the state machine role everything the ecs:runTask.sync integration needs to run and track the given task
 * definition. Mirrors the policy statements generated by stepfunctions_tasks.EcsRunTask.
 *
 * @internal
 */
export function grantEcsRunTask(scope: Construct, stateMachineRole: iam.IGrantable, task: ecs.TaskDefinition) {
  ...
  // grant on the unversioned task definition ARN so the latest revision can always be run
  ...
  // managed rule used by the runTask.sync integration; same statement for every ECS/Fargate provider so CDK
...
export class FargateRunnerProvider extends BaseProvider implements IRunnerProvider {
  ...
  /**
   * Shared state machine fragment that runs any Fargate provider, reading per-provider configuration generated by
   * {@link _runnerConfig} from `$.providerParams`. Mirrors what EcsRunTask used to render when each provider had a
   * dedicated state.
   *
   * @internal
   */
   ...
   * @internal
   */
  readonly _runnerFamilies: string[] = ['fargate'];
  ...
  /**
   * Runtime configuration for the shared Fargate family fragment.
   *
   * @internal
   */
  _runnerConfig(): any {
    // match EcsRunTask's default subnet selection: explicit selection, or public/private based on assignPublicIp
