  ...
  switch (error?.status) {
    case 401:
      return new RunnerConfigurationError(`GitHub rejected our credentials while registering a runner for ${target} (401). The personal access ` +
        'token may be expired or revoked, or the app private key may not match the app id. ' +
    ...
    case 403:
      return new RunnerConfigurationError(`GitHub denied us permission to register a runner for ${target} (403). Personal access tokens need ` +
        ...
        `${runnerLevel === 'org' ? 'the organization_self_hosted_runners permission' : 'the administration permission'}. ` +
        'See https://github.com/CloudSnorkel/cdk-github-runners/blob/main/SETUP_GITHUB.md');
    case 404:
      return new RunnerConfigurationError(`GitHub has no ${target}, or we have no access to it (404). ` +
        (runnerLevel === 'org'
          ...
          : 'Confirm the app is installed on this repository, or that the personal access token can access it.'));
    default:
      return undefined;
  }
}
