...
- **Self-signed certificates**: If GitHub Enterprise Server uses a self-signed certificate, you need to:
  1. Add the certificate to the runner image using `RunnerImageComponent.extra_certificates()`
  2. Add the certificate to the management functions using `extra_certificates` property on `GitHubRunners`
